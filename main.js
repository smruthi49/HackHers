...
let contractAddress = "0x78FEB112044c80410a33a8c3c4ef678F09BDBdf6";
let contractAbi = [
    {
      "inputs": [
        {
...
const addVote = async () => {
  if (WALLET_CONNECTED !== 0) {
      ...
      // Check if the current wallet address has already voted
...
const voteStatus = async() => {
    if(WALLET_CONNECTED !== 0) {
        ...
        const signer = provider.getSigner();
        const contractInstance = new ethers.Contract(contractAddress, contractAbi, signer);
        const currentStatus = await contractInstance.get_Voting_Status();
...
const getAllCandidates = async() => {
    if(WALLET_CONNECTED !== "") {
        ...
        var candidates = await contractInstance.get_all_votes();
        ...
        for (let i = 0; i < candidates.length; i++) {
            ...
            console.log(candidates[i].contractAddress);
        }
